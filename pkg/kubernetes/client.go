...
// InitClientWithInCluster 专门用于 in-cluster 模式初始化客户端
func InitClientWithInCluster() (*dynamic.DynamicClient, error) {
...
// InitClientWithKubeconfig 使用 kubeconfig 文件初始化客户端
func InitClientWithKubeconfig(kubeconfig string) (*dynamic.DynamicClient, error) {
	if kubeconfig == "" || kubeconfig == "~" {
		kubeconfig = filepath.Join("~", ".kube", "config")
	}
